...
combined_features = movies_data['genres']+' '+movies_data['keywords']+' '+movies_data['tagline']+' '+movies_data['overview']+' '+movies_data['production_companies']
...
def get_recommendations(movie_name):
    ...
    #Instead of using an 'index' column, get the row index directly
...
def get_recommendations_intersection(movie_name1, movie_name2):
    ...
    # Remove 'index' and 'crew' from the list of columns to drop
    # as they may not be present in your DataFrame
    final_suggestion=movies_data[movies_data['title'].isin(r_final['title'].tolist())].drop(['budget','homepage','id',
 'keywords',
 'original_language',
 'overview',
 'popularity',
 ...
 'vote_average',
 'vote_count'
 ], axis=1).reset_index(drop=True) # Use reset_index to drop the original index if needed
